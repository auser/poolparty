    ...
    plugin :poolparty do
      ...
      def enable                
        ...
        has_package(:name => "rubygems") do
          ...
          has_exec(:name => "build_messenger", :command => ". /etc/profile && server-build-messenger", :requires => get_gempackage("poolparty"), :onlyif => "/bin/ps | grep beam| grep node")
          has_exec(:name => "start_node", :command => ". /etc/profile && server-start-node", :requires => get_exec("build_messenger"), :ifnot => "/bin/ps | grep beam| grep node")
        ...
        execute_on_master do          
          ...
          has_exec(:name => "start master messenger", :command => ". /etc/profile && server-start-master", :requires => [get_gempackage("poolparty"), get_exec("build_messenger")], :ifnot => "/bin/ps | grep beam | grep master")
          
          has_exec(:name => "start client server", :command => ". /etc/profile && server-start-client", :requires => [get_gempackage("poolparty"), get_exec("build_messenger"), get_exec("start master messenger")], :ifnot => "/bin/ps | grep beam | grep client ")
