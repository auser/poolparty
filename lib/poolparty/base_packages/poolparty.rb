    ...
    plugin :poolparty do
      ...
      def enable                
        ...
        has_package(:name => "rubygems") do
          ...
          has_exec(:name => "build_messenger", :command => ". /etc/profile && server-build-messenger", :requires => get_gempackage("poolparty"), :onlyif => "test `/bin/ps aux | /bin/grep beam | /bin/grep node | wc -l` -eq 0")
          has_exec(:name => "start_node", :command => ". /etc/profile && server-start-node", :requires => get_exec("build_messenger"), :onlyif => "test `/bin/ps aux | /bin/grep beam | /bin/grep node | wc -l` -eq 0")
        ...
        execute_on_master do          
          ...
          has_exec(:name => "start master messenger", :command => ". /etc/profile && server-start-master", :requires => [get_gempackage("poolparty"), get_exec("build_messenger")], :onlyif => "test `/bin/ps aux | /bin/grep beam | /bin/grep master | wc -l` -eq 0")
          
          has_exec(:name => "start client server", :command => ". /etc/profile && server-start-client", :requires => [get_gempackage("poolparty"), get_exec("build_messenger"), get_exec("start master messenger")], :onlyif => "test `/bin/ps aux | /bin/grep beam | /bin/grep client | wc -l` -eq 0")
