...
start_link(Host, Port, Service, Options)
  when is_integer(Port), is_atom(Service), is_list(Options) ->
    ...
    {ok, TransportFactory} = thrift_socket_transport:new_transport_factory(Host, Port,  TransOpts),
    case thrift_socket_transport:new_transport_factory(Host, Port, TransOpts) of
      {ok, TransportFactory} ->
        case thrift_binary_protocol:new_protocol_factory(TransportFactory,  ProtoOpts) of
          {ok, ProtocolFactory} ->  start_link(ProtocolFactory, Service);
          Else                  -> 
            io:format("Error with new_protocol_factory(~p): ~p~n", [TransportFactory, Else]),
            Else
        end;    
      Else -> Else
    end.
...
call(Client, Function, Args) when is_pid(Client), is_atom(Function), is_list(Args) ->
